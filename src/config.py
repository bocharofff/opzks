...
_DEFAULTS: dict[str, Any] = {
    ...
    # Профиль экспорта "heatmap" — интерактивная HTML-карта (src/wifi_heatmap).
...
if __name__ == "__main__":
    ...
    USAGE = """
Показать итоговую конфигурацию и проверить права на файл точек доступа.

  python3 -m src.config [-h|--help]

Аргументов нет: печатает конфиг ровно в том виде, в каком его увидят
остальные модули, и предупреждает о небезопасных правах ap_targets.yaml.

Как это работает:
  * Настройки берутся из config/settings.yaml и накладываются поверх
    встроенных дефолтов. Файла может не быть — тогда работают дефолты.
  * Секции-словари (например heatmap:) сливаются ПО КЛЮЧАМ: можно задать
    в YAML один параметр карты и не потерять остальные её значения.
  * Файл с паролями точек (ap_targets.yaml) должен иметь права 600 —
    иначе выводится предупреждение (проверка, но не изменение прав).

Примеры:
  # убедиться, что правки в settings.yaml применились и видны программе
  python3 -m src.config

  # проверить, какие параметры карты реально используются
  python3 -m src.config | grep -A20 "'heatmap'"
""".strip()
