...
if __name__ == "__main__":
    ...
    USAGE = """
Самопроверка схемы БД на временной базе (демонстрация, не трогает рабочую базу).

  python3 -m src.db [-h|--help]

Аргументов нет: создаёт /tmp/test_wifi.db, наполняет её показательными
записями и печатает содержимое всех трёх таблиц.

Как это работает:
  * Проверяются ключевые свойства схемы: upsert сети сохраняет first_seen
    при повторной вставке; наблюдение без координат пишется с has_gps=0;
    результат проверки точки ложится в ap_health.
  * База открывается в WAL-режиме — он позволяет режиму 3 писать из двух
    потоков (мониторинг и проверка точек) одновременно.
  * Рабочая база (data/wifi_monitor.db) НЕ затрагивается: всё пишется во
    временный файл, который можно удалить сразу после просмотра.

Примеры:
  # убедиться, что схема создаётся и все три таблицы работают
  python3 -m src.db

  # заглянуть в рабочую базу вместо этого — обычным sqlite3
  sqlite3 data/wifi_monitor.db "SELECT COUNT(*) FROM observations;"
""".strip()
