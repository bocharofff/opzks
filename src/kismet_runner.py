...
if __name__ == "__main__":
    ...
    USAGE = """
Разовый прогон захвата: запустить Kismet, собрать данные, показать результат.

  sudo python3 -m src.kismet_runner <iface> [папка_логов] [каналы]

Аргументы:
  <iface>        интерфейс УЖЕ в monitor mode (напр. wlan0mon). Требуется root.
  [папка_логов]  куда Kismet пишет свой журнал (по умолчанию data/)
  [каналы]       список через запятую, напр. 1,6,11,36,44,149.
                 Без него Kismet сам перебирает все каналы (hopping)

Как это работает:
  * Модуль сам в monitor mode НЕ переводит — сделайте это заранее
    (src.interface_manager monitor <iface>) или запускайте режим 1 из cli.py.
  * Данные для карты берутся из таблицы packets журнала Kismet: у каждого
    кадра есть сигнал и координата — ГДЕ БЫЛ ПРИЁМНИК в этот момент. Именно
    поэтому получается карта покрытия вдоль маршрута, а не точка расположения AP.
  * Замеры прореживаются до одного сэмпла в секунду на сеть (bucket_sec):
    GPS всё равно обновляется примерно раз в секунду, а база растёт в разы медленнее.
  * Kismet сбрасывает данные на диск не сразу (интервал до ~30 с), поэтому
    скрипт ждёт появления базы и делает финальное чтение уже после остановки.
  * Ограничение каналов ускоряет накопление по нужному диапазону: при полном
    hopping на каждый канал приходится мало времени, и сети на 5 ГГц теряются.

Примеры:
  # быстрый прогон на всех каналах, журнал в data/
  sudo python3 -m src.kismet_runner wlan0mon

  # только 2.4 ГГц — чаще попадаются бытовые точки
  sudo python3 -m src.kismet_runner wlan0mon data/ 1,6,11

  # обычный рабочий путь: то же самое, но с БД, GPS и экспортом
