...
if __name__ == "__main__":
    ...
    USAGE = """
Проверка GPS: подключиться к gpsd и показать текущую позицию.

  python3 -m src.gps_monitor [host] [port]

Аргументы (оба необязательны):
  host   адрес gpsd (по умолчанию 127.0.0.1)
  port   порт gpsd  (по умолчанию 2947)

Как это работает:
  * Координаты берутся только у gpsd — само устройство модуль не открывает.
    Если gpsd не запущен, будет ошибка подключения, а не пустой результат.
  * Фикс появляется не мгновенно: холодный старт приёмника занимает минуты,
    поэтому скрипт ждёт 5 секунд и только потом печатает результат.
  * mode 2 — 2D-фикс (широта/долгота), mode 3 — 3D (есть высота). Ниже 2
    позиции нет вообще.
  * В рабочих режимах отсутствие фикса не останавливает сбор: наблюдения
    просто пишутся без координат (has_gps=0) и не попадают в карту.

Примеры:
  # проверить локальный gpsd перед выездом
  python3 -m src.gps_monitor

  # gpsd на другой машине/порту
  python3 -m src.gps_monitor 192.168.1.50 2947

  # если фикса нет — поднять gpsd вручную и посмотреть сырой поток
  sudo gpsd -N -G /dev/ttyACM0
  cgps -s
""".strip()
